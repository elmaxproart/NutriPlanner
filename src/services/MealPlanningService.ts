    ...
    try {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        for (const typeRepas of days) {
          const suitableRecipes = await Promise.all(recipes.map(async recipe => {
            const analysis = await this.geminiService.analyzeRecipe(recipe);
            const isSuitable = familyMembers.every(member => analysis.suitability[member.userId] === 'adapté');
          ...
          if (selectedRecipe) {
            const menu: Omit<Menu, 'id' | 'dateCreation' | 'dateMiseAJour'> = {
              ...
              recettes: [{ recetteId: selectedRecipe.id, portionsServies: familyMembers.length }],
            ...
            menus.push({ ...menu, id: menuId, dateCreation: formatDateForFirestore(new Date()), dateMiseAJour: formatDateForFirestore(new Date()) });
          }
        }
      }
  ...
  async optimizeMenu(menu: Menu, familyMembers: MembreFamille[]): Promise<Menu> {
    ...
    try {
      ...
      for (const recette of menu.recettes) {
        const analysis = await this.geminiService.analyzeRecipe({ id: recette.recetteId } as Recette); // Placeholder
      ...
      logger.error('Error optimizing menu', { error });
