...
interface SelectIngredientsModalProps {
  visible: boolean;
  onClose: () => void;
  onSelect: (ingredients: Ingredient[], isFinalStep?: boolean) => Promise<void>;
  title?: string;
...
const IngredientCard: React.FC<IngredientCardProps> = ({ ingredient, isSelected, onToggle }) => {
...
const SelectIngredientsModal: React.FC<SelectIngredientsModalProps> = ({
  visible,
  onClose,
  onSelect,
  ...
  useEffect(() => {
    if (visible) {
      ...
      AccessibilityInfo.announceForAccessibility('Modal de sélection d’ingrédients ouvert');
  ...
  const handleConfirm = useCallback(async () => {
    if (selectedIngredientIds.length > 0) {
      const selectedIngredients = dataSource.filter((i) =>
        selectedIngredientIds.includes(i.id)
      );
      await onSelect(selectedIngredients, isFinalStep);
      onClose();
      AccessibilityInfo.announceForAccessibility(`${selectedIngredientIds.length} ingrédient(s) sélectionné(s)`);
    }
  }, [selectedIngredientIds, onSelect, onClose, dataSource, isFinalStep]);
  ...
  return (
    <Modal
      ...
      accessible
      accessibilityLabel="Modal de sélection d’ingrédients"
      testID="select-ingredients-modal"
    >
